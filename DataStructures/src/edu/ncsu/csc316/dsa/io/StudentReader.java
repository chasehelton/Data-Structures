...
public class StudentReader {
	...
	private static Student processLine(String line) {
		Student s = new Student ("", "", 1, 1, 1.0, "");
		Scanner lineScanner = new Scanner(line);
		lineScanner.useDelimiter(",");
		while (lineScanner.hasNext()) {
			s.setFirst(lineScanner.next());
			s.setLast(lineScanner.next());
			s.setUnityID(lineScanner.next());
			s.setId(lineScanner.nextInt());
			s.setGpa(lineScanner.nextDouble());
			s.setCreditHours(lineScanner.nextInt());
		}
		lineScanner.close();
		return s;
	}
